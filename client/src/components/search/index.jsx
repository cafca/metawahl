...
class SearchComponent extends React.Component<SearchProps, SearchState> {
  ...
  constructor(props: SearchProps) {
    ...
    this.tagSearch = new Fuse(props.tags, tagSearchOptions)

    this.territorySearch = new Fuse(territoryList, territorySearchOptions)
  }

  componentWillReceiveProps(nextProps) {
    if (this.props.isLoading && nextProps.isLoading === false) {
      this.tagSearch = new Fuse(nextProps.tags, tagSearchOptions)
  ...
  handleResultSelect(e: ?Event, result) {
    ...
    const baseUrl =
      result != null && result.kind === "territory" ? "/wahlen/" : "/themen/"
  ...
  updateSearchResults(query) {
    ...
    if (query.length < 3) {
      ...
      tagResults = this.props.tags
        .filter(t => t.title && t.title.toLowerCase().startsWith(valueLower))
    ...
    if (query.length >= 3 || tagResults.length === 0) {
      ...
      tagResults.sort(
        (a, b) => -1 * (a.thesis_count / a.score - b.thesis_count / b.score)
      )
    }
  ...
  render() {
    ...
    const resultClassName =
      "results transition" + (this.state.query.length === 0 ? "" : " visible")
