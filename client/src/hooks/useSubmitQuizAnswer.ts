...
export interface SubmitQuizAnswerVariables {
  ...
  thesisNum: number;
  answer: QuizAnswer;
  /** Anonymous client id persisted in localStorage under the "uuid" key. */
  uuid: string | null;
}
...
// Submit is fire-and-forget with error logging — the response shape isn't
// consumed by callers. We decode whatever JSON comes back (if any) and hand
// it to the caller untouched.
