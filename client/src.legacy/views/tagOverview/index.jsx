...
import "../../index.css"
import { Container, Header, Icon, List, Loader, Grid } from "semantic-ui-react"
...
export default class TagOverview extends Component<RouteProps> {
  ...
  tagBySlug(slug: string): ?TagType {
    return this.props.tags.filter(t => t.slug === slug).shift(1)
  }
  ...
  render() {
    const sortByThesisCount = (tagA, tagB) => {
      return tagA.thesis_count === tagB.thesis_count
        ? tagA.slug < tagB.slug
          ? -1
          : 1
        : tagA.thesis_count > tagB.thesis_count
          ? -1
          : 1
    }
    ...
    const tagElems = rootTagNames.map((tag, i) => {
      const relatedTags =
        tag.related_tags.linked &&
        Object.keys(tag.related_tags.linked)
          .filter(k => rootTagNames.filter(t => t.title === k).length === 0)
          .map(k =>
            Object.assign({}, this.tagBySlug(tag.related_tags.linked[k].tag), {
      ...
      return (
        <Grid.Column key={i} className="revealMe">
      ...
      <Container className="tagList">
        ...
        <Grid relaxed divided doubling stackable padded columns={4}>
          <Grid.Row>
            ...
            <Grid.Column width={12}>
              ...
              <p>
                ...
                Hinter jedem von ihnen verstecken sich viele weitere
                Unterthemen. Jedes Thema ist dabei einem Eintrag auf Wikidata
                zugeordnet – einer Sammlung strukturierter Daten, die mit
                Wikipedia verknüpft ist.
              </p>
          ...
          {this.props.isLoading && (
            <Grid.Row textAlign="center" style={{ height: "10em" }}>
